...
#배경음악넣기
pygame.mixer_music.load(resource_path(os.path.join(current_path,"Eternal_Snow.mp3")))
pygame.mixer_music.play(-1,70)
pygame.mixer.music.set_volume(0.3)

#효과음 넣기
attack = pygame.mixer.Sound(resource_path(os.path.join(current_path,"laser_gun.wav")))
deadly = pygame.mixer.Sound(resource_path(os.path.join(current_path,"mario_die.wav")))
stage_clear = pygame.mixer.Sound(resource_path(os.path.join(current_path,"welcome_sound.wav")))
...
#배경 만들기
background = pygame.image.load(resource_path(os.path.join(current_path,"background.png")))

#스테이지 만들기
stage = pygame.image.load(resource_path(os.path.join(current_path,"stage.png")))
stage_size = stage.get_rect().size
stage_height = stage_size[1]  #스테이지의 높이 위에 캐릭터두기, 공 튕기게 하기위해.

#캐릭터 만들기
character = pygame.image.load(resource_path(os.path.join(current_path,"character_front.png")))
character_front = pygame.image.load(resource_path(os.path.join(current_path,"character_front.png")))
character_left = pygame.image.load(resource_path(os.path.join(current_path,"character_left.png")))
character_right = pygame.image.load(resource_path(os.path.join(current_path,"character_right.png")))
character_dead = pygame.image.load(resource_path(os.path.join(current_path,"character_dead.png")))
...
#무기 만들기
weapon = pygame.image.load(resource_path(os.path.join(current_path,"weapon.png")))
...
ball_images = [\
    pygame.image.load(resource_path(os.path.join(current_path,"balloon1.png"))),
    pygame.image.load(resource_path(os.path.join(current_path,"balloon2.png"))),
    pygame.image.load(resource_path(os.path.join(current_path,"balloon3.png"))),
    pygame.image.load(resource_path(os.path.join(current_path,"balloon4.png")))]
...
while running:
        ...
        for event in pygame.event.get(): #어떤 이벤트가 발생하였는가?
            ...
            if event.type == pygame.KEYDOWN:
                if event.key ==pygame.K_LEFT:
                    character = character_left      #캐릭왼쪽봄
                    ...
                    character = character_right     #캐릭오른쪽봄
                    right_pressed = True
                    if left_pressed:
                            character_to_x_LEFT = 0
                            left_pressed = False
    ...
    if character == character_dead:
        #아래에 if 부분없이 그냥입력하면 틱당 소리가 한번재생되는꼴이라 소리가 수십번겹쳐나옴,
        #그걸방지하기위해서 user_input값이 1일때, 0으로 바꾸고 소리를 재생시키도록 함.
        # 다음틱에 이부분을 실행할때는 이미 값이 0이므로 소리가 재생되지 않음. 
        if user_input == 1:
            user_input = 0
            deadly.play()
            deadly.set_volume(0.18)
        
        character_to_x_LEFT = 0
        character_to_x_RIGHT = 0
        character_y_pos += character_to_y_speed
        character_to_y_speed +=0.5
    ...
    if len(balls) ==0:      #balls리스트의 길이(내부 항목의 개수)가 0이면,
        ...
        pygame.mixer_music.stop()   #BGM종료후 클리어효과음재생
        stage_clear.play()
        stage_clear.set_volume(0.2)
        running = False
